...
export function expensesTable(expenses: MT_ExpenseInterface[]) {
  const table = `
      ...
      .map((expense, index) => {
        return `
                <tr style="border-bottom:  1px solid #61350615; transition: background-color 0.2s">
                  ...
                  <td style="padding: 12px 10px;">${dayMonthYear(expense.createdAt)}</td>
                  <td style="padding: 12px 10px;">${expense.description}</td>
                  <td style="padding: 12px 10px; color: #black; text-align: right; font-weight:bold; font-family: 'Courier New', monospace;">${'KES ' + expense.amount.toFixed(2)}</td>
                </tr>
              `;
      })
      .join("")}
